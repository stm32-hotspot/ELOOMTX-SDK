...
EPowerMode AppPowerModeHelper_vtblComputeNewPowerMode(IAppPowerModeHelper *_this, const SysEvent event)
{
  ...
  case SYS_PM_EVT_SRC_PB:
    ...
    {
      power_mode = E_POWER_MODE_TEST_ACTIVE;
    }
    else if (power_mode == E_POWER_MODE_TEST_ACTIVE)
    {
...
boolean_t AppPowerModeHelper_vtblCheckPowerModeTransaction(IAppPowerModeHelper *_this, const EPowerMode active_power_mode, const EPowerMode new_power_mode) {
  ...
  case E_POWER_MODE_STATE1:
    if ((new_power_mode == E_POWER_MODE_TEST_ACTIVE) || (new_power_mode == E_POWER_MODE_SLEEP_1)) {
      res = TRUE;
    }
    break;
  case E_POWER_MODE_TEST_ACTIVE:
...
//      __HAL_RCC_PWR_CLK_ENABLE(); //TODO: STF - check
  ...
  case E_POWER_MODE_TEST_ACTIVE:
    ...
    SYS_DEBUGF(SYS_DBG_LEVEL_VERBOSE, ("PMH: E_POWER_MODE_TEST_ACTIVE\r\n"));
